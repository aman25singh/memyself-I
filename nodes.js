document.addEventListener('DOMContentLoaded',function(){
    ...
    t_NodesJs.backgroundFrom = t.backgroundFrom;
    t_NodesJs.backgroundTo = t.backgroundTo;
    t_NodesJs.backgroundDuration = t.backgroundDuration;
    ...
    var mouseX = t.clientX; // Capture mouse position
    ...
    if (t_NodesJs.nodes.length) {
      ...
      // Move nodes away from the cursor if too close
    ...
    window[window.addEventListener ? "addEventListener" : "attachEvent"](window.addEventListener ? "load" : "onload", function() {
        ...
        var s = function() {
            ...
            t_NodesJs.nobg || (
                t = Math.floor((Math.sin(2 * Math.PI * Date.now() / t_NodesJs.backgroundDuration - Math.PI / 2) + 1) / 2 * (t_NodesJs.backgroundFrom[0] - t_NodesJs.backgroundTo[0] + 1) + t_NodesJs.backgroundTo[0]),
                o = Math.floor((Math.sin(2 * Math.PI * Date.now() / t_NodesJs.backgroundDuration - Math.PI / 2) + 1) / 2 * (t_NodesJs.backgroundFrom[1] - t_NodesJs.backgroundTo[1] + 1) + t_NodesJs.backgroundTo[1]),
                e = Math.floor((Math.sin(2 * Math.PI * Date.now() / t_NodesJs.backgroundDuration - Math.PI / 2) + 1) / 2 * (t_NodesJs.backgroundFrom[2] - t_NodesJs.backgroundTo[2] + 1) + t_NodesJs.backgroundTo[2]),
                d.beginPath(), d.fillStyle = "rgb(" + t + ", " + o + ", " + e + ")", d.fillRect(0, 0, n, i), d.fill()
            );

            // Calculate opposite color for lines and particles
            var lineR = 255 - t;
            var lineG = 255 - o;
            var lineB = 255 - e;
...
// Initialize NodesJs animation with configurations
      new NodesJs({
          id: "myCanvas",
          ...
          particleSize: 2,
          lineSize: 1,
          backgroundFrom: [250, 250, 250],
          backgroundTo: [192, 192, 192],
